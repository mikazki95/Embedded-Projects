...
#include <stdio.h>                                                              // Para acceder al API IPC RPMessage
#include <stdlib.h>
#include <string.h>                                                             // Para acceder al API IPC RPMessage
#include <inttypes.h>
#include <math.h>
...
#include <drivers/ipc_rpmsg.h>                                                  // Para acceder al API IPC RPMessage
...
#define LIMITE_BRADICARDIA           (1300)  // Periodo correspondiente a 55bpm
#define LIMITE_ASISTOLE              (4000)
// Definición de arritmias
#define NORMAL                          (0)
#define ASISTOLE                        (1)
#define FIB_VENTRICULAR                 (2)
#define TAQ_VENTRICULAR                 (3)
#define TAQUICARDIA                     (4)
#define BRADICARDIA                     (5)
#define CVP                             (6)
#define PAR CVP                         (7)
#define TV_MAYOR_2                      (8)
#define BIGEMINIA                       (9)
#define TRIGEMINIA                      (10)
#define LATIDO_PERDIDO                  (11)    //Se cambio por PAUSA
#define SIN_PULSO_MARCAPASOS            (12)
#define SIN_QRS_POST_MARCAPASOS         (13)
...
void generarTareas(void *args) /* función main */
{
    ...
    /* Iniciar el servicio RPMessage con el objeto RPMessage_Object. Este servicio se encargará de la recpción y envío de los mensajes IPC */
    ...
    TaskP_Params taskParams;                                                    // Parámetros para generar la tarea
    TaskP_Params_init(&taskParams);                                             // Iniciar parámetros. Recomendado por la API
    taskParams.name = "TAREA_01";                                               // Nombre para Debug
    taskParams.stackSize = IPC_RPMESSAFE_TASK_STACK_SIZE;                       // 8KiB
    taskParams.stack = gIpcTaskStack;                                           // Puntero al stack (*uint8_t)
    taskParams.priority = IPC_RPMESSAFE_TASK_PRI;                               // Prioridad de la tarea
    taskParams.args = &gIpcRecvMsgObject;                                       // Acá estamos pasando el RPMessage_Object como argumento de la función
    taskParams.taskMain = tarea1;                                               // Función que implementa la tarea

    /* Construir la tarea */
    status = TaskP_construct(&gIpcTask, &taskParams);
    DebugP_assert(status == SystemP_SUCCESS);
...
void tarea1(void *args)
{
    ...
    while(1)
    {
        ...
        do {
            ...
            gNumBytesWritten = 0U;
            trans.buf   = &gUartBuffer[0U];
            trans.count = sprintf(trans.buf,"d:%i,%i,%i,%i,%i,%i\r\n",dato_raw_d1,integral_ecg_d1,codigo_arritmia,time_segRR_actual,time_segRR_valido,variable00);
            transferOK = UART_write(gUartHandle[CONFIG_UART0], &trans);
            APP_UART_ASSERT_ON_FAILURE(transferOK, trans);
...
void EncontrarPuntoR(int32_t dato_in)       // entrada: Integral de ECG
...
void ProcesamientoSegmentoRR()
{
    ...
    float   delta_RR_real = 0,delta_RR_ant;
    ...
    else    // despues de los primeros 8 latidos analizados
    {
        ...
        {
            if (delta_RR_ant > delta_RR_real+10)
            cont_ritmo_irregular++;
            if ((delta_RR_real >= Thr_RR_miss)&& (delta_RR_real < LIMITE_ASISTOLE))// && (delta_RR_real < LIMITE_BRADICARDIA))       // DETECCIÓN DE LATID PERDIDO
            {
                cont_pulsos_brad++;
                if (cont_pulsos_brad > 1)
                    codigo_arritmia = BRADICARDIA;
                else //if ((delta_RR_real >= LIMITE_BRADICARDIA) && (delta_RR_real < LIMITE_ASISTOLE))
                    codigo_arritmia = LATIDO_PERDIDO;
            }

            else if (delta_RR_real <= LIMITE_TAQUICARDIA)
                codigo_arritmia = TAQUICARDIA;
            else if (delta_RR_real >= LIMITE_ASISTOLE)
                codigo_arritmia = ASISTOLE;
            delta_RR_ant = delta_RR_real;

        }
    }

    variable00 = (int)Thr_RR_miss;

    time_segRR_actual = (int)delta_RR_real;     //(float) media_RR_actual / 1000;
