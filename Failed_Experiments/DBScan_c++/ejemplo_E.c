...
int get_neighbors(int data[NUM_POINTS][2], int point[2], float eps, int neighbors[NUM_POINTS][2], bool visited[NUM_POINTS])
{
    ...
    for (i = 0; i < NUM_POINTS; i++)
    {
        ...
        delta_aux=abs(point[1] - other_point[1]);
        //if ((euclidean_distance(point, other_point) <= eps)&&(abs(point[0]-other_point[0])<3))
        if ( (!visited[i]) && (delta_aux <= eps)&&(abs(point[0]-other_point[0])<3))
        {
...
int expand_cluster(int data[NUM_POINTS][2],int neighbors[NUM_POINTS][2],float eps,bool visited[NUM_POINTS],int cluster[NUM_POINTS][2],int len_neighbors)//,int point[2],,   int minPts,  )
{
    ...
    for(i=0;i<len_neighbor_new;i++)
    {
        ...
        {
            int aux_cluster_ind = cluster[i][0];
            int aux_cluster_int = cluster[i][1];
            int aux_dato_ind = data[j][0];
            int aux_dato_int = data[j][1];
...
int find_cluster(int point[2], int clusters[NUM_POINTS_2][NUM_POINTS][2], int len_clusters[NUM_POINTS_2])
{
    ...
    for (i = 0; i < NUM_POINTS_2; i++)
    {
        if (len_clusters[i] > 0)
        {
            ...
            {
                if (abs(clusters[i][j][0] - point[0]) ==0 && abs(clusters[i][j][1] - point[1]) < 50)
                {
                    return i;
                }
            }
    ...
    for (int i = 0; i < long_clusters; i++)
    {
        if (clusters[i][1][1]>50)
        {
            ...
            for ( j = 0; j < len_clusters[i]; j++)
            {
                ...
                /*
                int index_pulso = clusters[i][j][0];
                if (index_pulso<=long_pulso)
                {
                    printf("pulso (%i , %i )\n", pulso[index_pulso][0], pulso[index_pulso][1]);  
                    printf("coster %i en: (%i , %i )\n", i, clusters[i][j][0], clusters[i][j][1]);  
                    if (abs(pulso[index_pulso][1]-clusters[i][j][1])<50)
                    {
                        aux_cont++;
                        break;
                    }  
            ...
            for ( j = 0; j < long_pulso; j++)
            {
                if ((pulso[j][0]<max_cluster0)&&(pulso[j][0]>min_cluster0)&&(pulso[j][1]<max_cluster1)&&(pulso[j][1]>min_cluster1))
                {
                    aux_cont++;
                    break;
                }
...
///////////////////////////////////////////////////////////////////////////////////
    ...
    num_clusters_1 = dbscan(data, eps, minPts, clusters, len_clusters, no_clusters, &len_no_clusters,NUM_POINTS);
    ...
    num_clusters_2 = dbscan(no_clusters, eps_2, minPts_2, clusters_2, len_clusters_2, no_clusters_2, &len_no_clusters_2,len_no_clusters);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
     ...
     for (int i = 0; i < NUM_POINTS; i++)
    {
        ...
        if (arritmia[i][0]==0)
        {
            printf("pulso %i:\n", cont_pulso);
            bool flag_arrit[200];
            for (int k = 0; k < contador_2; k++)
            {

                printf("(%i , %i )\n", pulso[k][0], pulso[k][1]);                   
            }
            flag_arritmia=find_arritmia(pulso,clusters,len_clusters,num_clusters_1,contador_2);
            cont_pulso++;
            contador_2=0;
        }
        pulso[contador_2][0]=arritmia[i][0];
        pulso[contador_2][1]=arritmia[i][1];
        contador_2++;
