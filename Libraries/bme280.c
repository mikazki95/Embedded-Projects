...
/**********************************************
 ...
 - Value 0x00 means BME280 undetected
...
uint8_t bme280_init(uint8_t sensor){
    ...
    switch (bme280_read1Byte(BME280_REGISTER_CHIPID, sensor)){
	   ...
	   i2c_inicia_com();
	   i2c_envia_dato(0xec|((1-sensor)<<1));
	   i2c_envia_dato(BME280_REGISTER_SOFTRESET);		
       i2c_envia_dato(0xB6);
	   i2c_detener();
	   ...
	   i2c_envia_dato(BME280_REGISTER_CONTROLHUMID);
	   i2c_envia_dato(BME280_HUM_CONFIG);
       break;
	...
	i2c_envia_dato(BME280_REGISTER_CONFIG);
	i2c_envia_dato(BME280_CONFIG);
    
    // write config for pressure, temperature and sensor-mode
	i2c_envia_dato(BME280_REGISTER_CONTROL);
	i2c_envia_dato((BME280_TEMP_CONFIG << 5)|(BME280_PRESS_CONFIG << 2)|(BME280_MODE_CONFIG));
	i2c_detener();
...
uint8_t bme280_read1Byte(uint8_t addr, uint8_t sensor){
	...
	i2c_inicia_com();
	i2c_envia_dato(0xec|((1-sensor)<<1)|0x01);
    value = i2c_read_nack();
    i2c_detener();
    return value;
}
